...
| Rsagacmd / SAGA | GIS-level processing wrappers | `saga$ta_morphometry$…` |
| Your workflow | Actual geospatial analysis | indices → mosaics → ML |
...
## Why This Matters for the Sentinel-2 Comparison Above

The workflows differ not because of STAC, but because each provider and wrapper implements data access differently (assets, authentication, preprocessing, formats).
STAC only standardises metadata — not how data is delivered or processed.

Therefore:

gdalcubes relies on consistent COG assets → works best with AWS/E84.

CDSE + GetImage uses ESA’s server-side logic → different behaviour.

terra manual uses STAC only for URLs → everything else is local.
...
Choosing a workflow is never merely technical.
It is a negotiation between the level of methodological precision, analytical depth, and scientific rigor you aim for — your "**scientific ambition**" — and the "**cognitive cost of plumbing**".
