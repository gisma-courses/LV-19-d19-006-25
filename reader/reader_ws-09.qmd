...
### S4 — Segment-based Pattern and Process Stratification and Station Candidate Derivation

The S4 module consumes the geometric base units generated in **S3 (segmentation and structural representation)** together with environmental reference layers from **S1 (DEM, land cover)** and aggregated wind statistics from the Wind layer.
Its purpose is not to redefine spatial structure, but to transform structurally defined segments into a constrained and interpretable **decision space for network design**.

For each segment, information-theoretic landscape metrics (entropy and relative mutual information) are computed to characterise spatial pattern complexity (pattern dimension).
In parallel, physiographic predictors (mean elevation, slope, southness, forest fraction) represent dominant process controls.
A seasonal wind exposure indicator (windwardness for summer and winter) is derived from terrain aspect and prevailing wind direction and retained as a descriptive process attribute.

Based on these predictors, two independent stratification systems are constructed: an IT-based pattern stratification and a physiography-based process stratification.
These strata still belong to the structural representation domain (S3); their role is to organise segments into comparable process and pattern classes.

The transition into the decision space (S4) occurs through the derivation of **station candidates**.
Within each stratum, representative segments are identified by minimising multivariate distance to the respective feature-space centroid.
The geometric centroids of these representative segments are exported as station candidate locations.
This procedure reduces the full spatial complexity to a compact but structurally and process-representative candidate set, which defines the constrained search space for subsequent optimisation and coupling steps (e.g. information gain, uncertainty reduction, hydrological or atmospheric consistency).
This separation preserves interpretability while keeping the optimisation problem computationally tractable.
