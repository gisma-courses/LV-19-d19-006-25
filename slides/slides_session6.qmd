...
This section introduces data access and prerequisites.
:::
...
This example shows how complex time series methods from external R packages can be applied in cloud computing environments using [`rstac`](https://cran.r-project.org/package=rstac) [@rstac] and [`gdalcubes`](https://cran.r-project.org/package=gdalcubes) [@gdalcubes]. We will use the [`bfast` R package](https://cran.r-project.org/package=bfast) containing unsupervised change detection methods identifying structural breakpoints in vegetation index time series. Specifically, we will use the `bfastmonitor()` function to monitor changes on a dedicated time period of a time series of Sentinel-2 imagery.
...
::: {.callout-success}
**Note.** Projecting to three different CRSs here is mainly for convenience/clarity. Only corner coordinates would actually be required, but having geometries ready in multiple CRSs simplifies later steps.
:::
...
# mapping the extents and boundaries of the chosen geometries
tmap_mode("view")
...
::: {.callout-success}
**Note.** The *gdalcubes image collection* acts as a proxy structure over the COGs.
:::
...
::: {.callout-success}
**Action.** Processing on the COG server starts only with a terminal action. Here: `write_ncdf`.
:::

```{r get_data_write_ncdf}
# "download" the data and write it to a netcdf file
...
Below you will find various examples dealing with the common NDVI and the [kNDVI](https://advances.science.org/doi/10.1126/sciadv.abc7447) indices.
...
The following function will iterate through 3 years of NDVI data and calculate NDVI difference maps for each pair of years.
...
ts = tibble(
  t0 = c("2018-04-01","2019-04-01","2020-04-01","2021-04-01"), 
...
system(paste(
  cmd,
  "-burn 1.0 -tr 10.0 10.0 -a_nodata 0.0 -te",
  st_bbox(ndvi[[1]])$xmin, st_bbox(ndvi[[1]])$ymin,
  st_bbox(ndvi[[1]])$xmax, st_bbox(ndvi[[1]])$ymax,
  "-ot Float32 -of GTiff",
  paste0(tutorialDir, "forest/MOF_mask_pr.shp"),
  paste0(tutorialDir, "forest/only_forestmask.tif")
))
...
tm_ndvi = lapply(seq(2:length(names_ndvi) - 1), function(i) {
  tm_shape(osm_forest) + tm_rgb() +
    ...
    tm_layout(
      panel.labels      = paste("Difference ", names_ndvi[[i]], "/", names_ndvi[[i+1]]),
      legend.show       = TRUE,
...
| :------------- | :---------------------------------- | :--------------------- |
| `apply_pixel`  | Vector of band values for one pixel | Vector of band values  |
| `reduce_time`  | Multi-band time series as a matrix  | Vector of band values  |
| `reduce_space` | 3-D array (bands, x, y)             | Vector of band values  |
| `apply_time`   | Multi-band time series as a matrix  | Multi-band time series |

There is almost no limit of what R function we use, but we must take care of a few things: 1) the reducer runs in a fresh R process (load packages inside), 2) it **must** always return a vector of constant length, 3) handle `NA`s.
  ...
  ncdf_cube("../data/MOF_10.nc") |>
    reduce_time(names = c("change_date", "change_magnitude", "kndvi"),
      FUN = function(x) {
        ...
        library(bfast)
        tryCatch({
          result = bfastmonitor(kndvi_ts, start = c(2020,1), history = "all", level = 0.01)
...
<iframe valign="center" src="m2.html" width="1024" height="960" frameborder="0" allowfullscreen="allowfullscreen"></iframe>

<figcaption>*Magnitude Change Map*</figcaption>

<p></p>

<iframe valign="center" src="m1.html" width="1024" height="960" frameborder="0" allowfullscreen="allowfullscreen"></iframe>
